...
def contest_list(requset):
	if request.method=='GET':
		username=request.user.username
		try:
			user_authority=Privilege.objects.get(user__user__username).authority
			...
			contests=Contest.select_related(depth=3).filter(Q(visible=True) | (Q(user__user__username=username) & Q(visible=False)))
		for contest in contests:
			if(contest.start_or_not() and contest.end_or_not) or (contest.start_or_not==False):
				...
				if not contest.defunct:
					...
					contest.save()
		return render_to_response("contest/contest_list.html",RequestContext(request,{'contests':contests}))		
...
def contest_rank(request):
	if request.method=='GET':
		...
		try:
			...
			error="比赛不存在"
			return render_to_response("error.html",RequestContext(request,{'error':error}))
		contest_user_id_list=Solution.objects.select_related(depth=3).filter(contest_id=cid).order_by('user__user__id').values_list('user__user__id',flat=True).distinct()
		...
		contest_problem_list=Contest_problem.objects.select_related(depth=3).filter(contest_id=cid)
		solutions=Solution.objects.select_related(depth=3).filter(contest_id=cid)
