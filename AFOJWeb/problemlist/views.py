...
def index(request):
	...
	return render_to_response("problemlist/problemlist.html",RequestContext(request,{"problem_list",problems}))
...
def problem_show(request,pid):
	if request.method=='GET':
		...
		try:
			authority=Privilege.obejects.get(user__user__username=username).authority
		except ObjectDoesNotExist:
			authority=None
			if cid==None:
				try:
					problem=Problem.obejects.get(problem_id=pid)
				except ObjectDoesNotExist:
					error="这个题目不存在哦"
		...
		if cid!=None:
			try:
				Contest_problem.obejects.get(contest_id=cid,problem_id=pid)
			except ObjectDoesNotExist:
				error="这个比赛或者题目不存在哦"
				return render_to_response("error.html",RequestContext(request,{"error":error}))
			try:
				contest=Contest.obejects.get(id=cid)
			except ObjectDoesNotExist:
				error="这个比赛不存在哦"
				return render_to_response("error.html",RequestContext(request,{"error":error}))
			if authority==config.ADMIN or problem.visible is True:
			...
			if contest.start_or_not():		
				if contest.private==0:
					...
					try:
						ContestPrivilege.obejects.get(contest_id=cid,user__user__username=username)
						return render_to_response("problemlist/problem.html",RequestContext(request,{"problem":problem,"cid":cid}))
					except ObjectDoesNotExist:
...
def submit_code(request):
	if request.method=='POST':
		...
		if len(code) == 0:
			...
			# return render_to_response("error.html",RequestContext(request,{"error":error}))
		if cid!=None:
			problem_obj=Contest_problem.obejects.get(contest_id=cid,problem_id=pid)
			submit_dic={
		...
		if cid !=None:
			try:
				...
				error="这个比赛不存在哦"
				return render_to_response("error.html",RequestContext(request,{"error":error}))
		solution=solution.obejects.create(**submit_dic)
		Source_code.obejects.create(solution=solution,source=code)
		if cid==None:
